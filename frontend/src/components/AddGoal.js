...
import styles from "../styles/AddGoal";
...
const AddGoalScreen = () => {
  ...
  const handleAddGoal = async () => {
    ...
    for (let i = 0; i < subTasks.length; i++) {
      ...
      if (isEmpty) {
        continue; // Allow completely empty tasks
      }
    ...
    try {
      const goalId = await addGoalToFirestore(goalTitle, combinedGoalDateTime);

      if (tasksToAdd.length > 0) {
        const tasksWithGroup = tasksToAdd.map((task) => ({
          ...task,
          goalId: goalId,
        }));
        await addMultipleTasksToFirestore(tasksWithGroup);
      }

      setShowSuccessNotification(true);
    } catch (error) {
      Alert.alert("Failure", "Failed to add goal");
    }
  };
        ...
        <View style={styles.notificationContainer}>
          <View style={styles.notificationBackground}>
            ...
            <Text style={styles.notificationMessage}>
              {goalTitle
                ? `“${goalTitle}” has been added to your goals.`
                : "Your new goal is now saved."}
            </Text>
